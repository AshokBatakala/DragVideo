...
from .renderer import Renderer, add_watermark_np
...
def init_images(global_state,w_load=None,stylegan2_wieghts_path=None,stage_1 = True):
    """
...
class DragVideo_Base:
    def __init__(self,
        ...
        # init image
        self.global_state = init_images(self.global_state  ,
                                        w_load=w_load,
    ...
    def run_old(self,N_STEPS = 10,points = None):
        """remove it later"""
        self.on_click_start(self.global_state,
                N_STEPS = N_STEPS,
                points = points,
    ...
    def drag_one_frame(self,w_load_path,
        ...
        lr = self.global_state['params']['lr']
        # self.load_latent(renderer,w_load_path,lr=lr)
        # #load points and run drag
        # torch.cuda.empty_cache()
        
        w_load = torch.load(w_load_path)#.to(renderer._device)
        self.global_state = init_images(self.global_state  ,
                                        w_load=w_load,
                                        stylegan2_wieghts_path=None,
                                        stage_1=False)
    ...
    def on_click_start(self,global_state,
        ...
        for _ in tqdm(range(N_STEPS)):
            ...
            if step_idx % global_state['draw_interval'] == 0:
                ...
                self.outputs["edited_latent"] = edited_latent.copy() # np.array
    ...
    @staticmethod
    def load_latent(renderer,
                    w_load_path,
                    w_plus = True,
                    w0_seed=None,
                    lr=0.001):
        """
        to be called before each latent;
        note: w_plus is kept True
        """
        # self = renderer
        
        w = torch.load(w_load_path).to(renderer._device)

        renderer.w0 = w.detach().clone()
        renderer.w_plus = w_plus
        if w_plus:
            renderer.w = w.detach()
        else:
            renderer.w = w[:, 0, :].detach()
        renderer.w.requires_grad = True
        renderer.w_optim = torch.optim.Adam([renderer.w], lr=lr)
